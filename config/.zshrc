...
if [[ -d $realpath ]]; then
    ...
    bat --color=always --line-range :150 $realpath 2>/dev/null || echo "无法预览该文件"
fi
'
#cd单独覆盖，保留空目录提示
zstyle ':fzf-tab:complete:cd:*' fzf-preview 'eza -1 --icons --color=always $realpath 2>/dev/null || echo "目录为空"'
...
typeset -g WORDCHARS=${WORDCHARS:s@-@}
typeset -g WORDCHARS=${WORDCHARS:s@\.@}
typeset -g WORDCHARS=${WORDCHARS:s@_@}
